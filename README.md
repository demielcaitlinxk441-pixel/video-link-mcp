...
| `WECHAT_CHANNELS_ALLOW_PUBLIC_WORKER` | 设为 `true` 后才允许将链接发送给默认公共 Worker | 默认 `false` |
...
### 模式 2：Worker API（需显式授权）

如果未配置元宝 Cookie，默认不会把视频号链接发送给公共服务。只有在 MCP 配置中显式设置 `WECHAT_CHANNELS_ALLOW_PUBLIC_WORKER=true` 时，才会调用公共 Cloudflare Worker API（`sph.litao.workers.dev`）。
...
{
  "env": {
    "WECHAT_CHANNELS_ALLOW_PUBLIC_WORKER": "true"
  }
}
```

返回结果中 `download_method` 字段为 `"wechat_channels_worker"`。如不希望使用公共 Worker，可配置元宝 Cookie 或自建 Worker。
