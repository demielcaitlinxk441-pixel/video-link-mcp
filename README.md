...
这是独立桌面窗口，不是网页。默认下载位置为“视频 / Video Link Analyzer”，也可在窗口中自行选择其他文件夹；选择会保存在当前电脑。下载记录同样只保存在本机。下载结束后会检查视频是否为 H.264 + AAC + yuv420p 的高兼容 MP4；不是时会自动生成并验证“兼容版”MP4。只有兼容版完整播放验证通过后，原始不兼容视频才会删除。
...
本项目采用 [MIT License](LICENSE) 开源。你可以使用、修改、发布和商用代码，但必须保留版权与许可证声明；软件按“现状”提供，不附带担保。
